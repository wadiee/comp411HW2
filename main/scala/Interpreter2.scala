...
////class Tuple(jamVal: JamVal, ast: AST) {
////  def getJamVal = jamVal
////  def getAST = ast
////}
//
//class Tuple(f: (AST, Map[Symbol, Tuple]) => JamVal, e: Map[Symbol, Tuple], ast: AST) {
//  lazy val jamVal = f(ast, e)
//  def getJamVal = jamVal
//  def getAST = ast
//}
//
...
//class Interpreter2(reader: java.io.Reader) {
...
//        var newMap = e
//        defs.map(d => (d.lhs.sym, new Tuple(helper, e, untilNotVariable(d.rhs, e)))).foreach(pair => newMap += pair)
//        helper(body, newMap)
//
//      // Constant
...
//          var newMap = en
//          vars.zip(args).map(pair => (pair._1.sym, new Tuple(helper, e, untilNotVariable(pair._2, e)))).
